...
func assertTest(name: String, json: String, type: Any.Type, path: String? = nil, expected: String) {
    let decoder = ZCJSONDecoder()
    let data = json.data(using: .utf8)!
    do {
        _ = try decoder.decode(type, from: data, path: path)
...
assertTest(name: "Deep Path: a.b.c", json: "{\"a\": {\"b\": {\"c\": {\"level1\": {\"level2\": {\"value\": \"OK\"} } }}}", type: DeepModel.self, path: "a.b.c", expected: DeepModel(level1: DeepModel.Level1(level2: DeepModel.Level2(value: "OK"))))
assertTest(name: "Invalid Path", json: "{\"data\": {\"user\": {\"name\": \"ZC\"}}}", type: User.self, path: "data.wrong", expected: "pathNotFound")
...
assertTest(name: "Empty Object", json: "{}", type: User.self, expected: "decodingError")
assertTest(name: "Malformed JSON", json: "{ invalid }", type: User.self, expected: "invalidJSON")
